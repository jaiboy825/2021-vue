...
<template>
  <div id="app" class="card-body">
    ...
    <input type="text" class="form-control" @keyup="keyUpEvent" />
    <br />esc:
    <input type="text" class="form-control" v-model="name" @keyup.esc="escEvent" />
    <br />Enter:
    <input type="text" class="form-control" v-model="msg" @keyup.enter="helpEvent"/>
    <br />
    <!-- 다른 조합도 적용됨 -->
    Alt + Enter
    <input type="text" class="form-control" v-model="msg" @keyup.alt.enter="showEvent" />
    <br />
    <br />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    keyUpEvent: function(evt) {
      console.log(evt);
      // if(evt.keyCode == 65) console.log('A가눌림')
